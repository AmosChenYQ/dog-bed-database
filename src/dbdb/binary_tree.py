...
class BinaryNodeRef(ValueRef):
    ...
    @property # called like class.prop
    def length(self):
...
class BinaryTree(LogicalBase):
    ...
    # Creates a new tree (Not unefficient)
    # The none afected part stays as always
    # The affected one generates new trees
        ...
        if node is None:
            ...
            new_node = BinaryNode.from_node(
                ...
                right_ref=self._insert(
                    self._follow(node.right_ref), key, value_ref)) # Create a new node with the insertion on children
        else:
